// ===== FIREBASE (Firestore) — buat notifikasi & testimoni realtime =====
    ...
    const firebaseConfig = {
        apiKey: "AIzaSyAkDVqV7dYQTaUSciOzqUnP0hQkvMYYHmc",
        authDomain: "belajar-34edb.firebaseapp.com",
        projectId: "belajar-34edb",
        storageBucket: "belajar-34edb.firebasestorage.app",
        messagingSenderId: "239538101383",
        appId: "1:239538101383:web:0455c5072a46aa50c42c22"
    };
    ...
    onSnapshot(query(collection(db, 'notifikasi'), orderBy('createdAt', 'desc')), (snap) => {
        ...
        updateNotifBadges();
        renderNotifPage();
        if (window.__tryShowNotifOverlay) window.__tryShowNotifOverlay();
    });
    ...
    // ----- Suara notifikasi (nada dibuat langsung lewat Web Audio, gak butuh file audio) -----
    function playNotifSound() {
        return new Promise((resolve) => {
            try {
                const ctx = new (window.AudioContext || window.webkitAudioContext)();
                const now = ctx.currentTime;
                const notes = [880, 1174.66]; // dua nada pendek, jernih ala notifikasi
                const noteDur = 0.16;
                const gap = 0.09;
                ...
                const totalMs = notes.length * (noteDur + gap) * 1000;
                setTimeout(() => { try { ctx.close(); } catch (e) {} resolve(); }, totalMs + 80);
            } catch (e) { resolve(); }
        });
    }

    // ----- Text-to-speech buat baca isi notifikasi (suara Indonesia paling jernih yang tersedia) -----
    let __ttsVoice = null;
    function pickClearVoice() {
        ...
        return idVoices.find(v => /google/i.test(v.name)) || idVoices[0] || voices.find(v => /google/i.test(v.name)) || voices[0];
    }
    if ('speechSynthesis' in window) {
        __ttsVoice = pickClearVoice();
        speechSynthesis.addEventListener('voiceschanged', () => { __ttsVoice = pickClearVoice() || __ttsVoice; });
    }
    ...
    // ----- Overlay preview notifikasi terbaru (muncul 1 detik setelah buka web) -----
    ...
    const __pageOpenTime = Date.now();
    let __notifOverlayShown = false;
    function maybeShowNotifOverlay() {
        if (__notifOverlayShown) return;
        try { if (sessionStorage.getItem('notifOverlaySeen')) return; } catch (e) {}
        if (!window.__notifData || window.__notifData.length === 0) return;
        const el = document.getElementById('notifOverlayPreview');
        if (!el) return;
        // Kalau data notif sudah siap tapi belum genap 1 detik, tunggu sisanya dulu
        const elapsed = Date.now() - __pageOpenTime;
        if (elapsed < 1000) { setTimeout(maybeShowNotifOverlay, 1000 - elapsed); return; }
        __notifOverlayShown = true;
        ...
        playNotifSound().then(() => speakNotifText(latest.desc || ''));
    }
    window.__tryShowNotifOverlay = maybeShowNotifOverlay;
